...
def plotHeatmap(Ps,rows,columns,xticks,yticks,titles,xlabelTitles,ylabelTitles,figuretitles,width_ratios,height_ratios,subfigdim,figsize=(10,10),filename=None,color="Greens"):
    ...
    for k, figrow in enumerate(subfigs):
        if subfigdim[1] > 1:
            ...
            axs=subfigs[k].subplots(1, subplotsInFig,gridspec_kw={"width_ratios":width_ratios,"height_ratios":height_ratios,'wspace': 0.0})
            for i, ax in enumerate(axs.flat):
                im = ax.imshow(np.array(Ps[k][i]), cmap=color,vmin=0,vmax=1)
                ax.set_xticks(np.arange(Ps[k][i].shape[1]))
                ax.set_yticks(np.arange(Ps[k][i].shape[0]))
                ax.set_xticklabels(xticks[k][i],fontproperties=prop)
                ax.set_yticklabels(yticks[k][i],fontproperties=prop,rotation=90)
                ax.set_title(titles[k][i],fontproperties=prop)
                ax.tick_params(axis='both', which='both', length=0)
                ax.set_xlabel(xlabelTitles[k][i],fontproperties=prop, va="top")
                ax.set_ylabel(ylabelTitles[k][i],fontproperties=prop, rotation=90, va="bottom")
                subfigs[k].suptitle(figuretitles[k][0],fontproperties=prop,size=14)
    # Add colorbar
    # fig.tight_layout()
