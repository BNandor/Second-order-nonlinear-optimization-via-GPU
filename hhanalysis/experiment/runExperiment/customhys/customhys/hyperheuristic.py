...
class Hyperheuristic:
    ...
    def __init__(self, heuristic_space='default.txt', problem=None,problemId=None, parameters=None, file_label='', weights_array=None,recordsPath=None):
        """
        ...
        if isinstance(heuristic_space, list):
            ...
            with open(f'{os.path.dirname(__file__)}/collections/' + heuristic_space, 'r') as operators_file:
    ...
    def run(self):
        ...
        best_solution = np.copy(current_encoded_solution)
        ...
        _save_step(0, dict(encoded_solution=best_encoded_solution, performance=best_performance,
                           details=current_details['samples']), self.problemId,None,self.recordsPath)
        ...
        while (step <= self.parameters['num_steps']) and (stag_counter <= max_stag):
            ...
            _save_step(step, {
                    ...
                    'details': candidate_details['samples']
                }, self.problemId,None,self.recordsPath)
    ...
    def evaluate_metaheuristic(self, search_operators):
        ...
        for rep in range(1, self.parameters['num_replicas'] + 1):
            ...
            mh = Metaheuristic(self.problem, search_operators, self.parameters['num_agents'],
                               self.parameters['num_iterations'],self.currentEvaluations,self.file_label)

            # Run this metaheuristic
            self.currentEvaluations=mh.run()
        ...
        return self.get_performance(fitness_stats), dict(
            historical=historical_data, fitness=fitness_data, positions=position_data, statistics=fitness_stats,samples=fitness_data)
    ...
    def brute_force(self):
        ...
        for operator_id in range(self.num_operators):
            ...
            _save_step(operator_id, {
                ...
                'statistics': operator_details['samples']
            },self.problem, self.file_label,self.recordsPath)
    ...
    def basic_metaheuristics(self):
        ...
        for operator_id in range(self.num_operators):
            ...
            _save_step(operator_id, {
                'encoded_solution': operator_id,
                'performance': operator_performance,
                'samples': operator_details['samples']
            },self.problem, self.file_label,self.recordsPath)
...
def _save_step(step_number, variable_to_save, problem,prefix='',recordsPath=None):
    """
    ...
    # else:
    #     folder_name = 'data_files/raw/' + str.replace(' ','',problem['name'])
    folder_name=recordsPath
    
    # Check if this path exists
    if not _check_path(folder_name):
        _create_path(folder_name)
    # path=f"{folder_name}{str.lower(problem['name'])}.json"
    path=f"{folder_name}/customhys.json"
    ...
    with open(path, 'w') as json_file:
            hash=f"{problem['name']}-{problem['dim']}"
            if not hash in data["experiments"]:
                data["experiments"][hash]={
                    "baseLevel-problemId":str.upper(problem['name']),
                    "baseLevel-xDim":problem['dim'],
                    "trials":[{
                    "med_+_iqr":variable_to_save['performance'],
                    'performanceSamples' :variable_to_save['details']
                    }]}
            else:
                data["experiments"][hash]["trials"].append({
                    "med_+_iqr":variable_to_save['performance'],
                   'performanceSamples' :variable_to_save['details']
                    })
            json.dump(data,json_file,cls=jt.numpyEncoder,indent=4)
