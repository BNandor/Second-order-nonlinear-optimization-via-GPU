...
class Experiment:
    ...
    def __init__(self, config_file=None, exp_config=None, hh_config=None, prob_config=None):
        ...
        if config_file:
            ...
            if directory == '':
                directory = f'{os.path.dirname(__file__)}/exconf/'  # Default directory
        ...
        self.exp_config = tl.check_fields(
            {
                ...
                'use_parallel': False,
                'parallel_pool_size': 12,  # Default
                'auto_collection_num_vals': 5,
                'recordspath': 'data_files/raw'
            }, exp_config)
        ...
        if not path.isfile(f'{os.path.dirname(__file__)}/collections/' + self.exp_config['heuristic_collection_file']):
            ...
            if exp_config['heuristic_collection_file'] in ['default.txt', 'automatic.txt', 'basicmetaheuristics.txt',
                                                           'test_collection']:
                raise ExperimentError(f'This collection name is reserved and cannot be created automatically!')
            else:
        ...
        if self.exp_config['weights_dataset_file'] and (
            ...
            if path.isfile(f'{os.path.dirname(__file__)}/collections/' + self.exp_config['weights_dataset_file']):
    ...
    def _simple_run(self, prob_dim,path):
        """
        ...
        hh = hyp.Hyperheuristic(heuristic_space=self.exp_config['heuristic_collection_file'],
                                problem=problem_to_solve, problemId={"name":function_string,"dim":num_dimensions},parameters=self.hh_config,
                                file_label=label, weights_array=weights,recordsPath=path)
