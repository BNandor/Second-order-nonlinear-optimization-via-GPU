...
class SimulatedAnnealingHyperLevel: public HyperLevel {
    ...
    double hyperOptimize(int totalEvaluations) override {
        ...
        for(int i=0; i < trials-1 || temp < 0; i++) {
            ...
            baseLevel.loadInitialModel();
            double currentFPrime=baseLevel.optimize(&currentMutatedByEpsilonParameters,totalEvaluations/trials);
            printf("f': %f trial %u \n",currentFPrime, i);
            printParameters(currentMutatedByEpsilonParameters);
            if(currentFPrime < currentF || std::uniform_real_distribution<double>(0,1)(generator)<exp((currentF-currentFPrime)/temp)) { // TODO add acceptance of worst performance with exponential probability
